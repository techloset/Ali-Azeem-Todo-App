...
const SignUp = ({navigation}: any) => {
  ...
  const handleSignUp = () => {
    // Check if any of the fields are empty or checkbox is not checked
    if (!name || !email || !phone || !password || !isChecked) {
      setError('Please fill in all fields and agree to the Terms & Conditions');
    } else {
      createUserAndAddToFirestore();
    }
  };
  ...
  return (
    <View style={signUpStyles.screen}>
      <View style={signUpStyles.container}>
        ...
        <View style={signUpStyles.bottom}></View>
        <ClickButton text="Signup" onPress={handleSignUp} />
        <View style={signUpStyles.bottomContainer}>
          <Text style={signUpStyles.bottomContainerText}>
            Already have an Account?
          </Text>
